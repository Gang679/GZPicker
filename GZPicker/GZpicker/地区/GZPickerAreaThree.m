...
@interface GZPickerAreaThree()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation GZPickerAreaThree

#pragma mark - --- init 视图初始化 ---

- (instancetype)initWithDelegate:(nullable id /*<GZPickerAreaThreeDelegate>*/)delegate
{
    self = [self init];
...
{
    if (!_arrayRoot) {
        NSString *path = [[NSBundle mainBundle]pathForResource:@"areaThree" ofType:@"plist"];
...
{
    if (!_toolbar) {
        _toolbar = [[GZPickerToolbar alloc]initWithTitle:@"选择城市地区"
...
- (UIView *)lineView
{
    if (!_lineView) {
        ...
        [_lineView setBackgroundColor:GZ_RGB(205, 205, 205)];
    }
    return _lineView;
}
